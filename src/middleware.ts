...
export function middleware(request: NextRequest) {
  ...
  // Password protect /status.html and /ff-ops
  if (pathname === "/status.html" || pathname.startsWith("/ff-ops")) {
    const STATUS_PASSWORD = process.env.STATUS_PASSWORD || "freshfacing2026";
    const cookie = request.cookies.get("status_auth")?.value;
    if (cookie !== STATUS_PASSWORD) {
